...
export default function SingleReportViewer({ 
  ...
  // Download function for the current document
  const handleDownload = () => {
    if (!report) return;
    
    try {
      if (shouldShowMarkdown && markdownContent) {
        // Download markdown content
        const blob = new Blob([markdownContent], { type: 'text/markdown' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `${report.name || 'document'}.md`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
      } else if (report.file && report.file instanceof File) {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `${report.name || 'document'}.pdf`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
      } else if (report.pdfUrl && report.pdfUrl.startsWith('blob:')) {
        ...
        console.warn('Unable to download: No suitable file source available');
          ...
          <div className="flex items-center gap-2">
            ...
            {report && (
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium bg-gray-700 hover:bg-blue-600 text-gray-300 hover:text-white border border-gray-600 hover:border-blue-700 transition-colors shadow-sm"
                title={`Download ${shouldShowMarkdown ? 'markdown' : 'PDF'} file`}
              >
                <svg
